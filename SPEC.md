...
- **填表页三件套对齐 tod（2026-09-23 晚，用户晒图点名「每一个游戏都要有」）**：①大 CTA=渐变流光主按钮（party-net 组件层 `.pn-create`「🌐 创建房间」+ `pn-shine` 扫光；bombcat `#btn-join` 同款 `bc-shine`——REDUCED 媒体块与 body.loperf 双退路关流光）；②「❓ 怎么玩？3 秒看懂」药丸：monopoly/uno 由宿主注入 `.pn-pills` 进 `.pn-form`（点击打开既有 `#rules-box` 并平滑滚到中央）；bombcat 既有 `#btn-guide` 换装药丸并改 tod 同款文案；③「☕ 喜欢的话，助力一下作者」药丸 + `#ovl-donate` 助力弹层（收款码 DONATE_QR base64 内嵌 200KB，从 tod 复用同一码，单文件零外链；点遮罩/「知道了」关闭）。取证 `.pw/shots/trio-*.png`。
- **防掉线双胞胎 + 刷新回房（2026-09-23 深夜，用户点名「防止设备掉线重连导致两个同样的用户，同时刷新页面可以回来」；探针 .pw/probe-rejoin.cjs 16 断言）**：① **party-net（monopoly/uno）**：`enter()` 加「同名回收」——房间里有同名玩家即掉线前的自己，座位过户给本标签新 id（`twin.id=myId`、清 bot、`cfg.onRejoin` 钩子让宿主回收游戏态座位=大富翁/UNO 掉线期间被代管的机器人座位还给人）；过户写 **`replaced` 墓碑名单**随文档广播，`netSetDoc` 的「成员合并防吞人」守卫跳过墓碑 id（否则别页会把掉线前的自己再加回来=3 人）；mount 尾**标签票据自动回房**（`sessionStorage :netroom`+`:netname-tab`——名字必须入标签票，共享 netname 会被后加入者覆盖=顶着别人名字进房）；`syncNetHUD` 收口 setup 显隐（`hidden = !!(onNet&&NDOC.started)`，boot 不再与自动回房竞态翻屏）。② **bombcat**：`join()` 已知 id 先认回座位（局中不再打成观战）；大厅**同名回收**；**房主回归接管**——刷新后 `S.hostId===myId && stage==='lobby'` 直接 becomeHost 并续用 retained seq/ver/成员表（否则 join act 无人应答=全房卡死）；回房票据 `cat:room`{room,ts,local}（10 分钟过期防复活死房；**local 标志随票**，checkbox 刷新会复位、不恢复会开出平行房间）；主动离开清票据。③ **tod/index**：doJoin 身份读取加 `tod:tab-ls`（localStorage 票兜底）——手动重进同房认回旧 id 不产生双胞胎；**boot 自动回房刻意只认 sessionStorage 标签票**（localStorage 票会劫持所有新标签页的加入表单=test-3d 稳定性翻车的根因）。语义红线：身份按标签页（sessionStorage），刻意不做 localStorage 全局 uid——同设备多标签=多玩家（本地多人/双开调试依赖）。门禁：probe-rejoin 16 断言（大厅/他人在线/开局中刷新回房、同名回收不涨员、主动退出不自动回房、uid 稳定）+ 全套既有门禁绿。
- **邀请链接三件套（2026-09-23，四游戏表单对齐 tod 口径）**：①大厅 `.pn-invite`「📋 复制邀请链接」= `origin+pathname+'?room='+房号`，剪贴板降级链 clipboard API→execCommand→失败 toast 报房号（file:// 退化为纯房号邀请文字）；②表单 `?room=` 5 位数字预填（脏参数静默忽略）；③房号输入框 maxLength 放宽 120 + paste 事件即时抽取（整段邀请链接/「房号 12345」文字），join 处理器 pnExtractRoom 兜底+回写规整值。**`localOnly`（?localnet=1）已脱离 AUTOTEST 门控**（monopoly/uno 各一行）——普通模式可开离线本地房，probe-room-link(8951) 依赖此参数。门禁：probe-room-link 29 断言（预填/脏参/复制/链接直进/粘贴抽号/不存在房间负例/双页互 join/带房号落地引导+名字框聚焦/**开始游戏仅房主可见**）。**带房号落地引导对齐 tod**（用户点名「直接页面链接过来」）：party-net 改 pn-sub 为「已带上房号 X —— 输入名字即可加入」+ 聚焦名字框（**短轮询重试**——宿主 boot 会夺焦，单次 focus 不够）+ enterkeyhint=go + 名字框回车即加入；bombcat 同款 toast 提示+聚焦。探针纪律两条新雷：①bombcat 探针房号必须**随机化**——真实 broker 上上一轮跑留下的 retained 房间（30 分钟过期）会让固定房号加入幽灵房间、当不上房主；②bombcat 的 #chk-local 视觉隐藏，`page.check()` 会超时——用 evaluate 置 checked 才能确定性走本地链路；③探针自身 server.listen 加 EADDRINUSE 自动退让 +1（连续复跑撞端口）。
