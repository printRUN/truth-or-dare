...
(function () {
  ...
  window.__three.fxState = function () {   // 调试访问器（追加访问器约定）：表情气泡/飞蛋/声波环/蛋碎/出卡阶段，探针用
    return { bubbles: bubbles.length, eggs: eggs3d.length, cardPhase: st.phase, leanMax: LEAN_MAX,
      ...
      shells: shellPool.filter(p => p.active).length, faceSplats: faceSplats.length,
  ...
  function frame(t) {
    ...
    if (stage === 'drawing' && (st.phase === 'park' || st.phase === 'shown' || st.phase === 'flip' || st.phase === 'reback' || st.phase === 'refly')) {
      ...
      st.flyAt = now + 600; st.t0 = now; qDrawn = false;
    ...
    if (st.phase === 'fly') {   // 900ms：单正弦弧（顶 +0.5 单位）+ 飞行旋卡 ≤180°；落定 80ms 微弹由 pulse 起始承接
      const k = Math.min(1, (now - st.t0) / 900);
      const sx = DECK_POS.x - PIVOT.x, sz = DECK_POS.z - PIVOT.z;         // deckLocal
      const ex = 0, ez = CARD_L / 2;                                       // restLocal（枢轴系）
      actionCard.position.set(sx + (ex - sx) * k, CARD_T / 2 + Math.sin(Math.PI * k) * 0.5, sz + (ez - sz) * k);
    ...
    if (st.phase === 'refly') {   // 重抽②：牌收回牌堆（倒放飞卡弧线），落位无缝交给既有 fly 段重新发牌
      const k = Math.min(1, (now - st.t0) / 420);
      const sx = DECK_POS.x - PIVOT.x, sz = DECK_POS.z - PIVOT.z;
      actionCard.position.set(sx * k, CARD_T / 2 + Math.sin(Math.PI * k) * 0.35, CARD_L / 2 + (sz - CARD_L / 2) * k);
      actionCard.rotation.y = Math.sin(Math.PI * k) * 2.2;
      if (k >= 1) { st.phase = 'fly'; st.t0 = now; shadowDirty(); }
    }
