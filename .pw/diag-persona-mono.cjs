...
(async () => {
  ...
  for (let i = 0; i < 200 && Date.now() - t0 < 300000; i++) {
    ...
    if (i % 2 === 0) console.log(`⏱ ${String(Date.now() - t0).padStart(6)} H[s${sH.seq},t${sH.turn},${sH.phase},bar${+sH.barShown}] J[s${sJ.seq},t${sJ.turn},${sJ.phase},bar${+sJ.barShown}] mine=${mine === host ? 'H' : mine === join ? 'J' : '-'}`);
    if (!mine) {
        ...
        if (stuckN === 12) {
          ...
          for (const [tag, pg] of [['H', host], ['J', join]]) {
            const d = await pg.evaluate(() => {
              ...
              return { seq: n.doc && n.doc.seq, docTurn: n.doc && n.doc.game && n.doc.game.turn, docPh: n.doc && n.doc.game && n.doc.game.phase, G: { turn: window.__mono.state.turn, ph: window.__mono.state.phase }, docPlayers: n.doc && n.doc.players && n.doc.players.map(p => p.name), myId: n.myId, writer: n.doc && n.doc.writer, tsAge: n.doc && Date.now() - n.doc.ts };
