...
(async () => {
  ...
  await host2.goto(url('h2') + '&role=host', { waitUntil: 'domcontentloaded' });
  await host2.waitForFunction(() => window.__mono && window.__mono.net().joined, null, { timeout: 15000 });
  await join2.goto(url('j2') + '&role=join&name=小绿', { waitUntil: 'domcontentloaded' });
  ...
  const members = await host2.evaluate(() => window.__mono.net().doc.players.map(p => p.name + ':' + (p.bot ? 'bot' : 'h') + ':' + Math.round((Date.now() - (p.lastSeen || 0)) / 1000)));
