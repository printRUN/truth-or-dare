...
### Scene Setup
- **View**: 3D 舞台（CSS 3D）——`#app { perspective: 1400px }` 提供透视，`#world3d`（`.world`）是唯一的「摄影机 rig」，四个屏（join/lobby/game/result）都在这一层里；屏常态下不挂 3D 变换（只有换屏那 0.64s 绕立方体公共竖棱转 90°，见下方红线里的 `scene-in/out`），每屏的常态机位在 JS `Cam.base`
...
- **每屏入场/退场 = 绕立方体公共竖棱转 90°**（`@keyframes scene-in` / `scene-out`）：变换写成 `perspective(1200px) translate3d(0,0,-R) rotateY(θ) translate3d(0,0,R)`（R = 面宽的一半 = 立方体半径）——新屏从 `rotateY(var(--tx-a))` 转到 0，旧屏从 0 转到 `rotateY(calc(-1 * var(--tx-a)))`，θ=±90° 时面心落在 `(∓R, 0, -R)`，正是「前墙在 z=0、立方体中心在 z=-R」的左/右墙。转的是**面心竖棱**（不是面的边缘）：两面共用同一个 R 与同一个 θ 增量，公共棱全程焊死（起转时都在 `(W/2,0,0)`，转到 45° 时都在 `(0,0,0.207W)`），读起来才是一个刚体。θ=0 时三段平移/旋转互逆、是**单位阵**，元素渲染与 `transform: none` 逐像素相同，所以落位不需要清理任何东西。透视写在 transform 的 `perspective()` 里，**不去动 `.world` 的渲染上下文**（红线：`preserve-3d` 绝不开，`.world` 仍 `transform-style: flat`）。方向由 `.world.tx-back` 把 `--tx-a` 整体反号；两个动画都只在换屏那 **0.64s** 里动 transform，超时后摘 `entering/leaving`、**常态回到 `transform: none`**——命中测试与布局不受影响（旧版 `screen-in` 只动 opacity 的做法已废弃；更早那版「走廊景深穿行」的 `--tx-zin/--tx-zout/--tx-ry/--tx-tilt` 已删除）
- **实体背板 + 「面向镜头=被照亮」（2026-09 一镜到底 v2）**：`.screen.entering/leaving::before` 各背一块深色板（`body:not(.loperf)` 才挂，`z-index:-1`，inset -14px 盖住棱），转身时转角永远不透背景光球，两面读成实心刚体；背板 opacity 走独立关键帧 `plate-in/out`（入屏 0.9→0.4→0、出屏 0→0.34→0.9，纯 opacity、与转身方向无关），**基态必须写 `opacity:0`**——动画是 backwards 填充（非 forwards），结束瞬间回落基态，基态若是 1 会在「动画已完、类还没摘」的窗口里凭空亮出一块暗板（probe-one-take 实测逮到过）。光照语义是「面向镜头的面亮、转离的面变暗」，±90° 两个转身方向都成立
- **揭幕与转身同拍**：加载层揭幕从 0.5s 淡出压成 **0.26s**（`opacity+scale`，**不带 blur**——blur 是 paint-bound 大层，光球去 blur 的同款教训）；`armCube`/`playSceneEnter` 仍在 `hideLoading` **同步同拍**挂载（旧墙此刻刚丢 `.active`，`.leaving` 是它唯一的显示来源，晚一帧就「凭空消失再弹回」），揭幕间隙用 `animationDelay:140ms + fill-mode:backwards` 实现——延迟期两面各自停在 from 帧（旧墙正视不透明、新墙侧棱透明），幕布落下与起转读成同一条时间轴的两拍。**绝不许改成 JS setTimeout 推迟挂载**
- **窄屏转身独占视口**：`Cam.enter` 在 `innerWidth<760 || LAND.ui` 时推进幅度归零（只做「走向新屏常态」的必要位移，无过冲），桌面留一半呼吸量（z 12 / rx 0.6）——转场帧实测旧版过冲会把屏内容推出边界（玩家卡被裁出左缘）
- **为什么这拼得出真立方体、却仍然不用 `preserve-3d`**：两面能当同一个刚体的墙，靠的是「同一个 R + 两面同宽 + Δθ≡90°」这三点解析地保证铰链是同一个 3D 点，再加上 `.screen.leaving` 与入屏占**同一矩形、同一中心**。开 `preserve-3d` 不但没必要，反而会让 `#app { overflow-x: clip }` 与屏内 `overflow:auto` 容器静默把 3D 上下文拍平，且上面那条「世界层是带透视的单平面」的命中测试红线随之失守——`.pw/test-3d.cjs` 的 `transformStyle === 'flat'` 就是这条的回归门。
- **几何前提（以后动面宽/屏序的地方都要回头看这条）**：① `--tx-r` 取**入屏**面宽的一半，由 `armCube()` 实测 `offsetWidth` 写入 `.world`（不能用 `getBoundingClientRect`——那会带上 `.world` 常驻机位的投影，量到的是镜头缩放后的框）；② 出屏的内联 `width` 同时被拉到 `2R`、`.leaving` 不设 `max-width`——宽屏（`min-width:1440px`）下大厅是 1180px、其余三屏是 920px，半径若取**出屏**宽度，出屏是大厅时它一转出就被夹回 920（半宽 460 < 半径 590），自己的棱够不到铰链，两墙之间裂开 ~130px 的缝（缝里直接漏背景）；反过来入屏是大厅时两墙重叠 ~92px；③ 方向走 `SCENE_ORDER` 环上的**最短路**（`(新−旧+4)%4`，`>2` 判为回退），所以 `result→join` 是转 −1 个棱而不是 −3；步数为 2 的**对面对**（`join↔game`、`lobby↔result`）仍然只转 90°——DOM 里换屏时只有两面存在，真转 180° 会转出一瞬间所有面都侧棱对镜头的空场，而「转一面墙」的视觉语言下没人分得清这面墙标的是什么；④ 两面必须**同一 duration + 同一 easing**（`0.64s cubic-bezier(0.4,0,0.2,1)`）——旧版入/出各 0.8s / 0.46s 且缓动不同，做成刚体必在中途散架；⑤ 加载层还盖着时两面的起转**一起攒到揭幕**（`pendingSceneEnter` 里同时放 `armCube` 与 `playSceneEnter`），否则旧墙在层后面独自转完、揭幕时只剩新墙单边从侧面甩进来，读不出墙角（`scene-in` 的 opacity 在 22% 就坐实到 1 这条不变量继续保留：揭幕不会先露一段黑底；`scene-out` 在 55% 还留 0.92，让 45° 那面墙（投影仍有 ~70% 宽）能看清是在转出去）；⑥ `lastScreenName` 的**初值必须是开机时屏上已经是的那一面**（加入页），留 `null` 会让每局必经的 `join→lobby` 走到「没有出屏」的分支——旧墙不挂 `.leaving`（瞬间消失）、`--tx-r` 也不写（新墙没有转身半径），结果是一面墙单边甩进来而不是两面铰接转动
...
- **换屏 = 一镜到底 + 一面墙的转身**：`renderScreen()` 只在换屏时干活——`Cam.enter(新屏, dir)` 从当前机位续接（540ms 推进 → 660ms 缓收常态，**绝不瞬移**；窄屏/横屏无过冲推进，见红线），同时 `armCube(旧屏, 新屏)` 给两面挂上同一个 0.64s 转身：旧屏加 `.leaving`（绝对定位、叠在上层、不接点击）绕公共竖棱转出，新屏加 `.entering` 从侧墙转进来，`--tx-r`/`--tx-a` 与 `.world.tx-back` 见上面红线那三条。转身方向是 `SCENE_ORDER`（join→lobby→game→result 首尾相接的**环**）上的最短路，所以 `result→join` 是转回身一个棱。join→lobby→game→result 连起来读是一条连续镜头，每一次换屏是其中一次转身
...
- Avatar options: 加入页头像区三标签 —— **预设**（24 张 = 14 pixel-art 像素 + 10 shapes 假面，🎲 一键随机）/ **🎨 定制**（DiceBear 全部 31 风格自由选）/ **我的**（保存过的定制 + 上传照片，localStorage 持久化，下次访问还在，可删）
  - **窄屏入局（<620px，2026-09 v2）**：头像墙改「两行横向滚动带」（`#avatar-selector` grid 逐列流式 + 横向滚动 + 两端 12px 渐隐 mask 示能），名字/房间号/加入按钮全部收进首屏——旧版 28 颗头像折行 7 行把 CTA 顶出首屏两屏开外（走查实锤）
...
- [x] 换屏一镜到底取证（`.pw/probe-scene.cjs`）：逐帧采样换屏期间的世界层机位——无瞬移（前 8 帧 z 单调推进）、新屏有 3D 入场且旧屏同时在转身（同一 0.64s）、动画结束进屋态后每屏 `transform` 回到 `none` 且选卡仍可命中；加入页→大厅的入场确实在加载层收起那一刻才播；game→lobby 回退带 `tx-back`；`shots/scene-fwd-*.png` / `scene-back-*.png` 留帧
- [x] 一镜到底 v2 取证（`.pw/probe-one-take.cjs`，12 条断言全绿）：①揭幕间隙——join→lobby 的 140ms delay 里旧墙仍以 from 帧 opacity 1 在场（fill-mode:backwards）、新墙停在侧棱 opacity 0、两面共用同一 delay；②实体背板——转身中段 ::before opacity >0.2 在场、落位点（scene-in progress≥0.85）前已在关键帧内归零（基态 opacity:0，无「暗板闪变」）；③窄屏（390×844）换屏镜头无过冲（z 单调收敛 -24，min>-25.5）；④颁奖礼——首渲染挂 ceremony + veil 挂 body、podium DOM 序=名次序、`.pscore` 首帧即终值、心跳式 renderScreen 重绘走无类终态不重放。测试环境注意：`test-perf` 的两条帧率断言对机器负载敏感（2026-09-13 实测基线与改动版同样 ~38fps，A/B 证实为环境噪声），别当回归信号
- [x] 立方体转身取证（`.pw/probe-cube.cjs`）：换屏期间逐帧解两面各自的计算 `rotateY`（从 `matrix3d` 的线性部分 `atan2(m[8], m[0])` 解出，与平移/透视无关）——`θ_in − θ_out` 全程恒为 ±90°（前进/回退两组 + 窄屏 390 / 普通 1000 / 宽屏 1440 / 横屏 844 四档共 5 组，**最大偏差 0.00°**；这条是「一个刚体而不是两张各转各的纸」的判据，每面自己的角度看着都对，只有这个差值能识别出两张独立的纸）；两面投影区间不分离（最大分离 −4.0 ~ 0.0px，公共棱焊死、缝里不漏背景）；两面同宽且 = 2R（366 / 920 / 1180 / 820）；`--tx-r` 恰等于入屏面宽的一半（183.0 / 460.0 / 590.0 / 410.0px），落位后无残留内联宽度；中途两面同时可见（前进 10 帧、回退 7 帧，投影宽 >1px 且 opacity >0.05）；落位后无 `entering/leaving`、每屏 `transform` 回到 `none`、镜头定稳后几何漂移 0px、选卡中心 `elementFromPoint` 命中自身；全程零 JS 报错。`shots/cube-fwd-*.png` / `cube-back-*.png` 留帧——**中段那一帧是在页面里把动画冻住再截的**（`getAnimations().pause()`）：按角度 `waitForFunction` 返回后 Playwright 才去合成写盘，这 200~400ms 足以让 800ms 的转场走完，实测会把「转角中段」拍成 |θ|≈13°、两面几乎重合的纸（第一版就这么假绿过）
